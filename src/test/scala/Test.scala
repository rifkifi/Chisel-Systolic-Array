...
class SystolicArrayTest extends Properties("SystolicArray") with ChiselScalatestTester {
    property("store and forward values correctly") {
        forAll { (valueIn: Int, keyIn: Int) =>
            test(new SystolicArray()) { dut =>
                ...
                // Test storing values
                dut.io.cmd.poke(1.U)
                dut.io.valueIn.poke(valueIn.U)
                dut.io.keyIn.poke(keyIn.U)
                dut.clock.step()
            }
        }
    }
